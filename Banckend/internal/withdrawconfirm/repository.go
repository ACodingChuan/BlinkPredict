...
func (r *Repository) UpsertSubmitted(ctx context.Context, cmd protocol.WithdrawConfirmCommand) (Submission, error) {
	if r == nil || r.pool == nil {
		return Submission{}, fmt.Errorf("withdraw submissions repository is not configured")
	}
	row := r.pool.QueryRow(ctx, `
		INSERT INTO withdraw_submissions (signature, wallet_address, amount_units, status, created_at, updated_at)
