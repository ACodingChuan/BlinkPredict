...
var logger = logging.New("withdrawconfirm")

const defaultConsumerName = "withdraw-confirm"

const (
	withdrawFetchBatch         = 16
	withdrawMaxWait            = 1500 * time.Millisecond
	withdrawRecoveryRetryBase  = time.Second
	withdrawRecoveryRetryLimit = 20 * time.Second
)
...
func NewService(client *natsjs.Client, pool *pgxpool.Pool, cfg config.Config, routers ...chainconfirm.WSRouter) *Service {
	rpcClient := logging.NewSolanaRPCClient("withdrawconfirm-rpc", cfg.SolanaRPCURL)
	...
	return &Service{
		...
		retryBase:    withdrawRecoveryRetryBase,
		retryLimit:   withdrawRecoveryRetryLimit,
	}
}
...
func (s *Service) ensureSubscription() error {
	...
	sub, err := s.client.PullSubscribe(protocol.SubjectWithdrawConfirm, s.consumerName)
	if err != nil {
		return fmt.Errorf("withdraw confirm subscribe: %w", err)
	}
	s.sub = sub
	return nil
}
...
func (s *Service) recoverActive(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("withdraw confirm recover active: %w", err)
	}
...
func (s *Service) run(ctx context.Context) {
	...
	for {
		...
		msgs, err := s.sub.Fetch(withdrawFetchBatch, nats.MaxWait(withdrawMaxWait))
		if err != nil {
			if errors.Is(err, nats.ErrTimeout) || errors.Is(err, context.DeadlineExceeded) {
				continue
			}
			logger.Warnf("withdraw confirm fetch failed: %v", err)
...
func (s *Service) handleMessage(msg *nats.Msg) {
	var cmd protocol.WithdrawConfirmCommand
	if err := json.Unmarshal(msg.Data, &cmd); err != nil {
		logger.Warnf("withdraw confirm decode failed: %v", err)
		_ = msg.Term()
		return
	}
	...
	if err != nil {
		...
		logger.Warnf("withdraw confirm load failed signature=%s err=%v", cmd.Signature, err)
...
func (s *Service) reloadSubmission(signature string) (Submission, bool) {
	...
	if err != nil {
		...
		logger.Warnf("withdraw confirm reload failed signature=%s err=%v", signature, err)
		return Submission{}, false
	}
	return updated, true
}
...
func (s *Service) retryBaseDelay() time.Duration {
	...
	return withdrawRecoveryRetryBase
}
...
func (s *Service) retryLimitDelay() time.Duration {
	...
	return withdrawRecoveryRetryLimit
}
...
func (s *Service) processSubmission(submission Submission) (retry bool, terminal bool) {
	...
	if err := s.repo.MarkWatching(opCtx, submission.Signature); err != nil {
		...
		logger.Warnf("withdraw confirm mark watching failed signature=%s err=%v", submission.Signature, err)
		return true, false
	}
	...
	if err != nil {
		if err := s.publishFailed(opCtx, submission.Signature, submission.WalletAddress, "invalid_signature"); err != nil {
			if s.isStoppingError(err) {
				return true, false
			}
			logger.Warnf("publish withdraw invalid-signature failed signature=%s err=%v", submission.Signature, err)
			return true, false
		}
		if err := s.repo.MarkFailed(opCtx, submission.Signature, "invalid_signature"); err != nil {
			...
			logger.Warnf("withdraw mark failed invalid-signature failed signature=%s err=%v", submission.Signature, err)
			return true, false
		}
		return false, false
	}
	...
	if err != nil {
		...
		if err := s.publishFailed(opCtx, submission.Signature, submission.WalletAddress, reason); err != nil {
			if s.isStoppingError(err) {
				return true, false
			}
			logger.Warnf("publish withdraw failed event failed signature=%s err=%v", submission.Signature, err)
			return true, false
		}
		if err := s.repo.MarkExpired(opCtx, submission.Signature, reason); err != nil {
			...
			logger.Warnf("withdraw mark expired failed signature=%s err=%v", submission.Signature, err)
			return true, false
		}
		return false, true
	}
	verified, err := VerifyWithdrawTransaction(opCtx, s.rpc, s.cfg, submission)
	if err != nil {
		if s.isStoppingError(err) || isRetryableVerifyError(err) {
			logger.Warnf("withdraw verify deferred signature=%s err=%v", submission.Signature, err)
			return true, false
		}
		logger.Warnf("withdraw verify failed signature=%s err=%v", submission.Signature, err)
		if err := s.publishFailed(opCtx, submission.Signature, submission.WalletAddress, "transaction_not_withdraw"); err != nil {
			if s.isStoppingError(err) {
				return true, false
			}
			logger.Warnf("publish withdraw verify-failed event failed signature=%s err=%v", submission.Signature, err)
			return true, false
		}
		if err := s.repo.MarkFailed(opCtx, submission.Signature, "transaction_not_withdraw"); err != nil {
			if s.isStoppingError(err) {
				return true, false
			}
			logger.Warnf("withdraw mark failed verify-failed failed signature=%s err=%v", submission.Signature, err)
			return true, false
		}
		return false, true
	}
	...
	if err := s.client.PublishJSON(opCtx, protocol.SubjectWithdrawConfirmed, verified.Signature, protocol.WithdrawConfirmedEvent{
		...
		logger.Warnf("publish withdraw confirmed failed signature=%s err=%v", verified.Signature, err)
		return true, false
	}
	if err := s.repo.MarkConfirmed(opCtx, verified.Signature, verified.Slot); err != nil {
		if s.isStoppingError(err) {
			return true, false
		}
		logger.Warnf("withdraw mark confirmed failed signature=%s err=%v", verified.Signature, err)
		return true, false
	}
	return false, true
}
...
func (s *Service) publishFailed(ctx context.Context, signature, wallet, reason string) error {
	...
	return s.client.PublishJSON(ctx, protocol.SubjectWithdrawFailed, signature+":"+reason, protocol.WithdrawFailedEvent{
		Signature:     signature,
