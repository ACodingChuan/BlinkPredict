...
const (
	...
	depositRecoveryRetryBase  = time.Second
	depositRecoveryRetryLimit = 20 * time.Second
)
...
func NewService(client *natsjs.Client, pool *pgxpool.Pool, cfg config.Config, routers ...chainconfirm.WSRouter) *Service {
	...
	return &Service{
		...
		retryBase:    depositRecoveryRetryBase,
		retryLimit:   depositRecoveryRetryLimit,
	}
}
...
func (s *Service) reloadSubmission(signature string) (Submission, bool) {
	...
	if err != nil {
		...
		logger.Warnf("deposit confirm reload failed signature=%s err=%v", signature, err)
		return Submission{}, false
	}
	return updated, true
}
...
func (s *Service) retryBaseDelay() time.Duration {
	...
	return depositRecoveryRetryBase
}
...
func (s *Service) retryLimitDelay() time.Duration {
	...
	return depositRecoveryRetryLimit
}
...
func (s *Service) processSubmission(submission Submission) (retry bool, terminal bool) {
	...
	verified, err := VerifyDepositTransaction(opCtx, s.rpc, s.cfg, submission)
	if err != nil {
		if s.isStoppingError(err) || isRetryableVerifyError(err) {
			logger.Warnf("deposit verify deferred signature=%s err=%v", submission.Signature, err)
			return true, false
		}
		logger.Warnf("deposit verify failed signature=%s err=%v", submission.Signature, err)
		if err := s.publishFailed(opCtx, submission.Signature, submission.WalletAddress, "transaction_not_deposit"); err != nil {
			if s.isStoppingError(err) {
				return true, false
			}
		...
		if err := s.repo.MarkFailed(opCtx, submission.Signature, "transaction_not_deposit"); err != nil {
			if s.isStoppingError(err) {
				return true, false
			}
	...
	if err := s.client.PublishJSON(opCtx, protocol.SubjectDepositConfirmed, verified.Signature, protocol.DepositConfirmedEvent{
